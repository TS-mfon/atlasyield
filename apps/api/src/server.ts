...
app.use(
  (
    ...
    console.error("[atlasyield-api] unhandled route error", error);
    res.status(500).json({
...
const bootstrap = async () => {
  ...
  app.listen(env.API_PORT, () => {
    console.log(`AtlasYield API listening on :${env.API_PORT}`);
  });
};

bootstrap().catch((error) => {
  console.error("Failed to start AtlasYield API", error);
  process.exit(1);
});
